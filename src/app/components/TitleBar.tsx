...
import { Minus, Square, X, Wifi, Shield, Activity, Maximize2 } from "lucide-react";
...
export function TitleBar() {
  ...
  const { activeAlt, trafficStats } = useUiData();
    ...
    >
      <div className="flex items-center gap-3 px-4 h-full">
        <div className="flex items-center gap-2">
          ...
          <span
            className="text-[#404040] tracking-[0.1em]"
            style={{ fontSize: "10px", fontFamily: "'Inter', sans-serif", fontWeight: 400 }}
          >
            CONTROL CENTER
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            {activeAlt ? `${activeAlt.name} В· ${activeAlt.status.toUpperCase()}` : "NO ACTIVE ALT"}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6 absolute left-1/2 -translate-x-1/2">
        <div className="flex items-center gap-1.5">
          <Wifi size={10} className="text-[#555555]" />
          <span style={{ fontSize: "10px", fontFamily: "'JetBrains Mono', monospace", color: "#888888" }}>
            {trafficStats.downloadMbps.toFixed(1)} Mbps
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ fontSize: "10px", fontFamily: "'JetBrains Mono', monospace", color: "#888888" }}>
            {trafficStats.latencyMs.toFixed(0)}ms
          </span>
        </div>
      </div>
